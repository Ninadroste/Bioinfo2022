...
# Results 
## 1. VSN 
After the QC, we proceeded to normalize our data using the variance stabilization method (VSN). To see if the VSN was successful we used two plotting techniques, the mean versus standard deviation plot (meanSD) and the density probability against the log intensity plot. The meanSD plot, shows a slight upward trend of the red line which represents the median estimator. If all chips were of good quality, the median estimator would show a horizontal line. 
...
Through R, we were able to matchour mouse data set to the TRA dataset provided by Dinkelacker. Upon matching, we found that our data set contains 6188 TRAs. After that the tras were matched with their respective tissue to create a dataframe that contains the ensemble transcripts of the TRAs, their expression value in each microarray chip and their tissue.
...
Using a volcano plot, shows the statistical significance versus magnitutde of fold change. The genes with a deviating fold change can be seen either side of the expected fold change. The genes highlighted in red have a significant change of expression values. 
```{r}
EnhancedVolcano(dge_df.m, x = "logFC", y ="adj.P.Val", lab = rownames(dge_df.m),
...
```
We saw that the most amount of upregulated TRAs are found between the fourth and eigth cell stage 
````{r}
barplot(summary(results.m)[3,],las =2,col =brewer.pal(5,"Pastel2"), ylab="sum of upregulated genes")
````

After that we plotted the TRA frequency against the different tissues and saw that the oocyte has the most amount of TRAs after that comes the heart.
...
In total we have 149 TRAs in the heart, 62 of those are overexpressed. 
In order to visualize the tissues with most expressed TRAs we used a treemap to categorize the large tissues and to visualize the total share of tissues present in the fourth and eight cells stage.
````{r, echo=FALSE}
...
png(file = "treemap_foureight_oe.png", height = 800, width =800)
...
Using a VENN diagram, we saw that there are three TRAs differenatially expressed that can be found in cell stage two to four, four to eight, and eight to morula. 
